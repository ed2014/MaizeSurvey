...
Figure 8. Most commonly experienced, early- and late-sowing dates reported by respondents per region. Points are individual responses and lines indicate range of responses. Calculation assumes mid-points of sowing date for each category.
...
```
Figure 10. Range of hybrid maturity selection based on Comparative Relative Maturity (CRM). Maximum and minimum CRM values selected by respondednts were classified as early- and late-maturity choices for a given region. Points are individual responses and lines indicate range of responses. Calculation assumes mid-points of CRM categories.
...
data_clean %>%
 ...
 # mutate(CRMcat = as.factor(CRMcat)) %>%
  ...
  mutate(RegionName =  reorder(RegionName, value)) %>%
  ggplot(aes(x=RegionName, y=value)) +
  geom_line(size=3, alpha=0.3, aes(colour=Category),position = position_dodge(width = 0.5)) +
 # geom_boxplot(aes(colour=Category),position = position_dodge(width = 0.5), alpha=0.5) +
  geom_point(aes(colour=Category),
...
* Wet soil was the most common factor delaying sowing of maize crops
