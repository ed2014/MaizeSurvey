...
A total of XXX maize growers in the FAR database were contacted by email and asked to contribute to an online survey on crop management and yields (Appendix). 

The main aim of the survey was to provide a nationwide overview of crop performance and the most common management practices utilised. In addition, the data was used to develop a prototype model able to estimate regional differences in crop yields across the country (Figure 1).
...
The map shows the "aproximate" region of respondents (actual locations were modified to ensure anonymity of responses). The map shows categories of silage yield simulations by the APSIM-model parameterised using data from the survey (e.g. ranges of sowing dates).

![Figure 1. Silage yield simulation categories by APSIM-maize and approximate survey respondent regions.](Respondents_YieldCat.tiff)
...
#unique(data_clean$YearsAsProducer)
...
The range in sowing dates, for early-, most commonly experienced and late-sowing conditions, is shown in Figure 8.
...
#  geom_point(aes(colour=Category), width = 0.2, alpha = 0.5, size = 2) +
  facet_grid(Category~.) + # Comment out for a single graph
  coord_flip() + 
...
Figure 8. Most commonly experienced, early- and late-sowing dates reported by respondents per region. Lines indicate range of responses. Calculation assumes mid-points of sowing date for each category.
...
The most common factor that delay sowing date was excessive soil moisture in spring, with the exception of Canterbury where low temperatures (including risks of frost) was the main delay factor (Figure 9).
...
Figure 9. Main factors contributing to a delay in the time of maize sowing. Number of producers that selected each category. "Low temperatures" category includes frost risk and low soil temperatures. The category "others" includes lack of available machinery and vary dry soil conditions.
...
#  geom_jitter(aes(colour=Category), width = 0.2, alpha = 0.5, size = 2) +
  geom_jitter( width = 0.1, 
               ...
               shape = 1,
               aes(colour=Category)) +
# geom_point( aes(colour=Category), position = position_jitterdodge()) +
  scale_shape_discrete(solid=F) +
#  facet_grid(Category~.) + # Comment out for a single graph
  coord_flip() + 
...
Figure 10. Distribution of respondent selection for maize hybrid maturity as Comparative Relative Maturity (CRM). Maximum and minimum CRM values selected were classified as early- and late-maturity choices for a given region. Lines indicate range of responses. Calculation assumes mid-points of sowing date for each category by combining "most commonly used" and "sometimes used" categories.
...
The overall insights from this survey were:
* Sowing dates and genotype maturities were widely variable, even within similar regions
* Wet soils were the most common factor delaying sowing of maize crops
* Most maize growers utilise conventional cultivation methods
* Silage yields average ~20 t/ha; mostly ranging between 15 and 25 t/ha
* Grain yields average ~12 t/ha; mostly ranging between 9 and 15 t/ha


## Acknowledgements
We sincerely thank the maize producers who contributed to this survey. 